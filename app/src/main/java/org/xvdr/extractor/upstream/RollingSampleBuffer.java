package org.xvdr.extractor.upstream;
...
public final class RollingSampleBuffer {
    ...
    public void appendData(Packet p, int length) {
        while (length > 0) {
            int thisAppendLength = prepareForAppend(length);
            p.readBuffer(lastAllocation.data, lastAllocation.translateOffset(lastAllocationOffset), thisAppendLength);
    ...
    public void commitSample(long sampleTimeUs, int flags, long position, int size) {
        infoQueue.commitSample(sampleTimeUs, flags, position, size, null);
    }
